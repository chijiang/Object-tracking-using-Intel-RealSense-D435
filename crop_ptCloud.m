function ptCloud = crop_ptCloud(ptCloud, bbox)   
    ...
    u_l_corner = [(bbox(1) - 640) * 0.9025/(1280/...
    (2 * tan(70/180*pi/2))), -(720 - bbox(2)...
    - bbox(4) - 360) * 0.9025/(720/...
        (2 * tan(70/180*pi/2)))];
    % Position of the lower right corner.
    l_r_corner = [(bbox(1) + bbox(3) - 640) * ...
    0.9025/(1280/(2 * tan(70/180*pi/2))),...
    -(720 - bbox(2) - 360) * 0.9025/(1280/...
        (2 * tan(70/180*pi/2)))];
    ...
    ptCloud = pcdenoise(ptCloud, 'Threshold',0.001);
	%------------- END OF CODE --------------
end