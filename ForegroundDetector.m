function [Ishape,bbox,centroid, bbox_color,depth_uint8] = ForegroundDetector(crop_depth,background,BlobAnalysis,crop_color)
      Ishape = [];
      bbox_color = []; 
      % konvertierung des Tiefenbildes in einem Binärbild mit Datentyp uint8
      depth_uint8 = TiefenbildBinarisierung(crop_depth, 200); 
...
%           bbox_color = [round(bbox(1)*scale1),round(bbox(2)*scale2),round(bbox(3)*scale1),round(bbox(4)*scale2)];
          Ishape = insertShape(crop_color,'rectangle',bbox,'Color',...
            'green','Linewidth',6); 
      end
end 